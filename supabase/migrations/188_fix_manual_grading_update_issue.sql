-- Fix the manual grading update issue
-- This migration adds better error handling and debugging to identify why updates fail
...
BEGIN
  ...
  IF update_count = 0 THEN
    -- Additional debugging: Check if the attempt still exists
    SELECT EXISTS(
      SELECT 1 FROM standalone_quiz_attempts WHERE id = input_attempt_id
    ) INTO attempt_exists;
    
    IF NOT attempt_exists THEN
      RAISE EXCEPTION 'Attempt % was deleted during the grading process', input_attempt_id;
    ELSE
