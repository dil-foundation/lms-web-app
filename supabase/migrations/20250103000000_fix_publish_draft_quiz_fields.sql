-- Migration: Fix publish_draft function to include missing quiz question fields
-- Description: Updates the publish_draft function to include all quiz question fields (points, image_url, question_type, math fields)
-- Date: 2025-01-03
...
COMMENT ON FUNCTION "public"."publish_draft"("draft_id_in" "uuid", "published_id_in" "uuid") IS 'Safely synchronizes content from a draft course to a published course, preserving student progress and including all metadata fields (country_ids, region_ids, city_ids, project_ids, board_ids, school_ids, class_ids). FIXED: Now includes all quiz question fields (points, image_url, question_type, math_expression, math_tolerance, math_hint, math_allow_drawing).';
