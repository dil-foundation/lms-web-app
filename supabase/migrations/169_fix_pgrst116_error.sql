-- Fix PGRST116 error by ensuring get_standalone_quiz_with_questions always returns a result
-- This prevents "JSON object requested, multiple (or no) rows returned" error
...
COMMENT ON FUNCTION public.get_standalone_quiz_with_questions(uuid) IS 'Gets a quiz with its questions and options - fixed PGRST116 error by returning NULL values when quiz not found';
