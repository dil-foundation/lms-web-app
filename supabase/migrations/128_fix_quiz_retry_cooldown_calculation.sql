-- Fix quiz retry cooldown calculation to handle decimal hours
-- The issue was that retryCooldownHours was being cast to INTEGER, 
-- but it should be NUMERIC to handle decimal values like 1/60 (1 minute)

-- Update the can_retry_quiz function to properly handle decimal cooldown hours
...
DECLARE
    ...
    cooldown_hours NUMERIC; -- Changed from INTEGER to NUMERIC
...
BEGIN
    ...
    -- Check if study materials are required (removed since we removed this feature)
    -- This section was removed as part of the advanced settings cleanup
...
COMMENT ON FUNCTION can_retry_quiz(UUID, UUID) IS 'Fixed to handle decimal cooldown hours (e.g., 1/60 for 1 minute) instead of casting to integer';
