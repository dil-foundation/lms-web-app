-- Simplified manual grading function without auth.users access
-- Removes teacher validation to avoid permission issues
...
BEGIN
  ...
  RAISE NOTICE 'Attempt % exists, quiz_id: %', input_attempt_id, attempt_quiz_id;
  ...
  EXCEPTION
    WHEN foreign_key_violation THEN
      RAISE EXCEPTION 'Foreign key constraint violation when updating attempt %. Check if teacher_id % exists. Error: %', input_attempt_id, input_teacher_id, SQLERRM;
    WHEN OTHERS THEN
