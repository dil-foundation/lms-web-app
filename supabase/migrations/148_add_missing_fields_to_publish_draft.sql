-- Migration: Add missing fields to publish_draft function
-- Description: Syncs countries, regions, cities, projects, boards, and classes fields
-- Date: 2024-12-31
-- Version: 1.0

-- Drop the existing function to recreate it with additional fields
DROP FUNCTION IF EXISTS publish_draft(uuid, uuid);

-- Recreate the function with the missing fields
CREATE OR REPLACE FUNCTION publish_draft(draft_id_in uuid, published_id_in uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
GRANT EXECUTE ON FUNCTION publish_draft(uuid, uuid) TO service_role;
