-- Migration: Update publish_draft function to include payment fields
-- Description: Adds payment_type and course_price fields to the publish_draft function UPDATE statement
-- Date: 2025-01-17
...
BEGIN
  ...
  -- AND payment fields: payment_type, course_price
  ...
  SET
      ...
      -- STRIPE PAYMENT FIELDS: Sync payment settings from draft
      payment_type = draft.payment_type,
      course_price = draft.course_price,
      updated_at = now()
        ...
        IF rec_content_item.content_type = 'quiz' THEN
          ...
          ELSE
            ...
            FOR rec_question IN SELECT * FROM public.quiz_questions WHERE lesson_content_id = rec_content_item.id ORDER BY position LOOP
              -- FIXED: Include all quiz question fields including points, image_url, question_type, and math fields
              ...
              VALUES (
                match_content_item_id, 
                rec_question.question_text, 
                rec_question.position,
                COALESCE(rec_question.question_type, 'single_choice'),
                COALESCE(rec_question.points, 1),
                rec_question.image_url,
                rec_question.math_expression,
                rec_question.math_tolerance,
                rec_question.math_hint,
                COALESCE(rec_question.math_allow_drawing, false)
              )
...
-- Update the function comment to reflect the addition of payment fields
COMMENT ON FUNCTION "public"."publish_draft"("draft_id_in" "uuid", "published_id_in" "uuid") IS 'Safely synchronizes content from a draft course to a published course, preserving student progress and including all metadata fields (country_ids, region_ids, city_ids, project_ids, board_ids, school_ids, class_ids) and payment fields (payment_type, course_price).';
