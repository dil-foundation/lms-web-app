-- Fix the teacher_id foreign key constraint issue
-- The UPDATE is failing because the teacher_id doesn't exist in auth.users or has wrong format
...
BEGIN
  ...
  SELECT EXISTS(
    SELECT 1 FROM standalone_quiz_attempts WHERE id = input_attempt_id
  ) INTO attempt_exists;
  ...
  RAISE NOTICE 'Attempt % exists, proceeding with grading', input_attempt_id;
  ...
  -- Debug: Log the attempt details
  RAISE NOTICE 'Attempt details - quiz_id: %, total_questions: %, text_answer_questions: %', 
    attempt_quiz_id, total_questions, text_answer_questions;
  ...
  EXCEPTION
    WHEN foreign_key_violation THEN
      RAISE EXCEPTION 'Foreign key constraint violation when updating attempt %. Check if teacher_id % exists in auth.users', input_attempt_id, input_teacher_id;
    WHEN OTHERS THEN
