...
CREATE TRIGGER trigger_initialize_user_progress AFTER INSERT ON auth.users FOR EACH ROW EXECUTE FUNCTION initialize_user_progress();
...
using ((bucket_id = 'dil-lms'::text));
  ...
  for insert
  to authenticated
with check ((bucket_id = 'dil-lms'::text));



  create policy "Authenticated users can upload to dil-lms-public sax7ka_0"
  on "storage"."objects"
  as permissive
  for insert
  to authenticated
with check ((bucket_id = 'dil-lms-public'::text));



  create policy "Public read access for dil-lms-public sax7ka_0"
  on "storage"."objects"
  as permissive
  for select
  to public
using ((bucket_id = 'dil-lms-public'::text));



  create policy "Users can delete their own files in dil-lms-public sax7ka_0"
  on "storage"."objects"
  as permissive
  for delete
  to authenticated
using (((bucket_id = 'dil-lms-public'::text) AND ((storage.foldername(name))[1] = 'avatars'::text) AND ((storage.foldername(name))[2] = (auth.uid())::text)));



  create policy "Users can update their own files in dil-lms-public sax7ka_0"
  on "storage"."objects"
  as permissive
  for update
  to authenticated
