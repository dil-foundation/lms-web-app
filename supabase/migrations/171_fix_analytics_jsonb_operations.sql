-- Migration: Fix analytics functions to handle correct answers data structure
-- The answers field is stored as an object (not array), so we need to fix the JSON operations

-- Fix get_question_performance_analytics function
...
COMMENT ON FUNCTION public.get_question_performance_analytics(uuid) IS 'Gets performance analytics for each question in a quiz - fixed to handle object-based answers structure';
