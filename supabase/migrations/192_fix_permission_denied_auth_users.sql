...
-- Remove direct access to auth.users and use a different validation approach
...
BEGIN
  ...
  -- Validate teacher by checking if they exist in profiles table (instead of auth.users)
  -- This avoids permission issues with auth schema
  IF NOT EXISTS(SELECT 1 FROM profiles WHERE id = input_teacher_id) THEN
    RAISE EXCEPTION 'Teacher % does not exist in profiles table', input_teacher_id;
  END IF;
  
  RAISE NOTICE 'Teacher % exists in profiles', input_teacher_id;
  ...
  EXCEPTION
    WHEN foreign_key_violation THEN
      RAISE EXCEPTION 'Foreign key constraint violation when updating attempt %. Check if teacher_id % exists in profiles. Error: %', input_attempt_id, input_teacher_id, SQLERRM;
    WHEN OTHERS THEN
