-- Script: Fix Access Logs Column Name
-- Description: Fixes the admin_disable_mfa_for_user function to use the correct column name
-- Date: 2024-01-15
...
BEGIN
  ...
  -- Log the action (using the correct column name 'metadata' instead of 'details')
...
EXCEPTION
  WHEN OTHERS THEN
    -- Log the error (using the correct column name 'metadata' instead of 'details')
    INSERT INTO access_logs (user_id, user_email, action, ip_address, user_agent, status, metadata)
    VALUES (
      admin_user_id,
      admin_user_email,
