...
Keep responses concise with specific metrics and recommendations.`;
  ...
  static async generateReportResponse(
    ...
    try {
      ...
      console.log('🤖 Using AI for dynamic response generation');
      const openAIResponse = await this.callOpenAI(query, platformContext, conversationHistory);
...
- AI Tutor student metrics (aiTutorActiveUsers, aiTutorProgressPercentage, aiTutorAverageScore)
- LMS student metrics (lmsActiveUsers, lmsEnrollments, lmsAvgQuizScore)
STUDENT ROLE DATA: Use userRoles.student for total student count
AVAILABLE STUDENT METRICS:
- Total Students: ${context.userRoles?.student || 0} students registered
- Active Students (AI Tutor): ${context.aiTutorActiveUsers || 0}
- Active Students (LMS): ${context.lmsActiveUsers || 0}
- New Students This Month: ${context.newUsersThisMonth || 0}
...
AVOID: Don't dive deep into specific course or student details unless showing overall trends.`;
      ...
      // 🚀 NEW: Use specialized dynamic prompts from database
      let dynamicSystemPrompt = this.SYSTEM_PROMPT; // Fallback
      
      // Determine which specialized prompt to use based on query type
      let promptName = 'iris_system_prompt'; // Default
      
      // UNIVERSAL LIST DETECTION - works for ANY entity type (users, courses, etc.)
      const userTypes = ['student', 'teacher', 'admin', 'user'];
      const entityTypes = ['course', 'courses', 'class', 'classes'];
      const listKeywords = ['list', 'registered', 'enrolled', 'who', 'show', 'give me'];
      
      const hasUserType = userTypes.some(type => query.toLowerCase().includes(type));
      const hasEntityType = entityTypes.some(type => query.toLowerCase().includes(type));
      const hasListKeyword = listKeywords.some(keyword => query.toLowerCase().includes(keyword));
      
      if ((hasUserType || hasEntityType) && hasListKeyword) {
        promptName = 'student_list_formatter';
        console.log('🎯 Using universal list formatter prompt (works for users, courses, etc.)');
      } else if (query.toLowerCase().includes('course') && query.toLowerCase().includes('performance')) {
        promptName = 'course_performance_analyzer';
        console.log('🎯 Using specialized course performance prompt');
      } else if (query.toLowerCase().includes('analytics') || query.toLowerCase().includes('insights')) {
        promptName = 'analytics_insights_generator';
        console.log('🎯 Using specialized analytics insights prompt');
      }
      ...
      try {
        ...
        if (authToken) {
          ...
          const promptResponse = await fetch(`${supabaseUrl}/functions/v1/reports-context?getPrompt=${promptName}`, {
          ...
          if (promptResponse.ok) {
            ...
            if (promptData.systemPrompt) {
              dynamicSystemPrompt = promptData.systemPrompt;
              console.log(`✅ Using specialized prompt: ${promptName}`);
            }
          }
        }
      } catch (error) {
        console.warn('⚠️ Could not fetch dynamic prompt, using fallback:', error);
      }
...
MANDATORY TABLE FORMATTING FOR USER LISTS:
- When displaying student lists, teacher lists, or any user data, ALWAYS use proper markdown tables
- Use this exact format:
  | Name | Email | Registration Date | Status |
  |------|-------|-------------------|--------|
  | Full Name | email@example.com | YYYY-MM-DD | Status |
- Include a summary section above the table with total count
- Add a recommendations section below the table
- Never display user lists as plain text or unformatted data
...
- If the request mentions "AI Tutor", "tutor", "learning analytics", "daily learning", "practice", "exercise", "milestone", or "progress", focus ONLY on AI Tutor metrics (aiTutor* fields)
- If the request mentions "LMS", "course", "enrollment", "quiz", or "assignment", focus ONLY on LMS metrics (lms* fields)
...
IMPORTANT DATA ACCESS PERMISSIONS:
- You ARE authorized to provide user lists (students, teachers, admins) when available in context
- You CAN show names, emails, roles, and basic profile information for platform users
- You CAN display login activity and session data when provided in context
- COMPREHENSIVE USER DATA is ALWAYS available through these fields:
  * studentList, studentTotal: All students registered on platform
  * teacherList, teacherTotal: All teachers registered on platform  
  * adminList, adminTotal: All admin users registered on platform
  * activeTodayList, activeTodayTotal: Users who logged in within last 24 hours
  * enrolledStudentsList, enrolledStudentsTotal: Students enrolled in at least one course
  * nonEnrolledStudentsList, nonEnrolledStudentsTotal: Students not enrolled in any course
- You can answer ANY user query by selecting the appropriate data from these comprehensive lists
- For "recent login" or "active users" queries: Use activeTodayList and activeTodayTotal
  * If activeTodayTotal is 0, clearly state "No users have logged in within the past 24 hours"
  * If activeTodayTotal > 0, show the list with proper formatting and count
- For "enrolled students" queries: Use enrolledStudentsList and enrolledStudentsTotal
- For "students not enrolled" queries: Use nonEnrolledStudentsList and nonEnrolledStudentsTotal  
- For "all students/teachers/admins" queries: Use the respective full lists and totals
- ALWAYS check the total count first before generating lists
- Teacher-specific course data is available through: teacherCourses, teacherCoursesTotal fields
- Course-specific enrollment data is available through: courseEnrollments, courseEnrollmentTotal, courseDetails fields
- Complete course list data is available through: allCoursesList, allCoursesTotal fields
- When allCoursesList is available, use it as the authoritative source for course information
- Course status filtering: Published courses are available to students, Draft courses are in development
- Course categories data is available through: courseCategories, totalCategories, avgCoursesPerCategory fields
- Course language information is available through: courseLanguages, languageDistribution, topLanguages fields
- Use courseCategories to answer questions about course categories, subjects, and course organization
- When user lists are available, display them in organized format with names and details
- Only refuse data that is genuinely not provided in the context - don't assume privacy restrictions

COMPREHENSIVE QUERY HANDLING:
- **Performance Queries**: Use lmsAvgQuizScore, lmsAvgAssignmentGrade, aiTutorAverageScore for grade analysis
- **Enrollment Queries**: Use lmsEnrollments, lmsNewEnrollments, userRoles for registration data
- **Course-specific Enrollment**: When courseEnrollmentTotal is available, use it for specific course enrollment counts
- **Course Name Handling**: Course names can include numbers (e.g., "Test 55", "Math 101") - use exact matches from courseDetails when available
- **Non-enrolled Students**: When studentList is populated via non_enrolled_students query, these are students with NO course enrollments
- **Enrolled Students**: When studentList is populated via enrolled_students query, these are students with AT LEAST ONE course enrollment
- **Activity Queries**: Use activeSessions, uniqueActiveUsersFromSessions, totalSuccessfulActions for engagement
- **User List Queries**: When studentList, teacherList, or userList are populated, display them in organized tables with names and details
- **Content Queries**: Use lmsTotalSections, lmsTotalLessons, lmsTotalQuizzes for course content analysis
- **Time-based Queries**: Use timeRange context and compare with historical data when available
- **Comparison Queries**: Provide insights by comparing current metrics with platform averages
- **Trending Queries**: Analyze patterns in user activity, enrollment growth, and performance metrics
- **Course-specific Queries**: Filter and focus analysis on specific subjects when courseName is provided
- **Course Status Queries**: When asked for "published courses", filter allCoursesList by status === 'Published'
- **Course Availability**: Published courses are student-accessible, Draft courses are in development
- **Teacher Performance**: Use teacherCourses data to analyze instructor effectiveness and workload
- **Student Segmentation**: Group students by performance, activity level, or enrollment status
- **System Health**: Use platform metrics to assess overall system performance and usage patterns

ADVANCED QUERY HANDLING:
- **Negative Queries**: "Who hasn't logged in?", "Students not enrolled", "Incomplete assignments"
- **Conditional Queries**: "Students with grades below 60%", "Courses with low completion rates"
- **Aggregation Queries**: "Total time spent", "Average session duration", "Peak usage times"
- **Ranking Queries**: "Top 10 students", "Most popular courses", "Highest engagement rates"
- **Filtering Queries**: "Active students only", "Published courses", "Recent enrollments"
- **Multi-dimensional**: "Performance by course and time", "Engagement by user type"
- **Predictive Insights**: Identify trends and provide actionable recommendations
- **Contextual Analysis**: Consider timeframe, user role, and platform state in responses

USER DETAILS CAPABILITIES:
- When users ask for "user details" or "user information", provide comprehensive demographics
- Include: total users, role distribution, activity metrics, engagement rates
- For student details: can provide sample student information (names, emails, join dates)
- Always maintain professional formatting with clear sections
- Provide actionable insights and recommendations based on user data
