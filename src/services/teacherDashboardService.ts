...
class TeacherDashboardService {
  ...
  async getProgressOverviewData(timeRange: string = 'all_time'): Promise<TeacherProgressOverviewData> {
    try {
      console.log('🔄 Fetching teacher dashboard progress overview with timeRange:', timeRange);
      ...
      const students = (data.students || []).map((student: any) => ({
        ...
        completionPercentage: student.completion_percentage || student.completionPercentage || 0,
        ...
        aiTutorFeedback: {
          summary: student.ai_feedback?.summary || student.aiTutorFeedback?.summary || 'No feedback available',
          sentiment: this.normalizeSentiment(student.ai_feedback?.sentiment || student.aiTutorFeedback?.sentiment),
          lastFeedback: student.ai_feedback?.last_feedback || student.aiTutorFeedback?.lastFeedback || 'No recent feedback'
        },
        performance: {
        ...
        flags: {
          ...
          inactive_days: student.flags?.inactive_days
        }
      }));
      ...
      const averageScore = totalStudents > 0
        ? Math.round(students.reduce((acc: number, s: any) => acc + s.averageScore, 0) / totalStudents)
        : 0;
      const studentsAtRisk = students.filter((s: any) => Object.keys(s.flags).some(key => s.flags[key] > 0)).length;
